...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for iteration in range(2**30):
            ...
            # perform inference
            predict = model(to_tensor(frames, use_cuda, use_half))

            # write all frames in a batch
            for success, frame, label in zip(states, frames, predict):
                if success:
                    text = classes[torch.argmax(label).item()]
                    cv2.putText(frame, text, (0, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
